import typing, discord, os, psycopg2
from discord import *
...
class Reactions(commands.Cog, name = "ReactionRoles"):
    ...
    async def reactionroles(self, ctx):
        await ctx.invoke(self.bot.get_command("help"), entity="reactionroles") #calling another command within the instance. Calling relevant commands
    
    @reactionroles.command(name="channel") # Setting a channel
    @commands.guild_only()
    @commands.has_guild_permissions(manage_channels=True) # Needing permissions within server to manage channels
    async def rr_channel(self, ctx, channel: discord.TextChannel = None):
        if channel is None:
            await ctx.send("No channel was given, hence I shall use the current channel I am within.")
    
        channel = channel or ctx.channel
        try:
            await channel.send("Testing to see if I can send messages within this channel.", delete_after= 10)
        except discord.HTTPException:
            await ctx.send("Sadly, I cannot send a message to that channel, please give me permissions to send a message and retry", delete_after = 30)
            return
        ...
        reaction_roles = list(filter(lambda r: r['guild_id'] == ctx.guild.id, reaction_roles)) # Reference creation with everything needed, filter object
        for item in reaction_roles:
    ...
    async def rr_toggle(self, ctx):
        ...
        await ctx.send(f"I have toggled that for you now! It is now currently {is_enabled}")
    ...
    async def rr_add(self, ctx, emoji:typing.Union[discord.Emoji, str], *, role: discord.Role):
        ...
        if len(reacts) >= 20:
            await ctx.send("This does not support more than 20 reaction role per server! Sorry!")
            return
