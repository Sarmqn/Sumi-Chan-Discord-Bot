...
class MyHelp(commands.HelpCommand):
    ...
    async def send_bot_help(self, mapping):
        ...
        for cog, commands in mapping.items():
            ...
            command_signatures = [
                self.get_command_signature(command) for command in filtered
            ]
    ...
    async def send_cog_help(self, cog):
        ...
        # If nothing was returned, i.e. user doesn't have access to use that cog
            ...
            await self.context.reply(
                f'No category called "{cog.qualified_name}" found.'
            )
    ...
    async def send_group_help(self, group):
        ...
        command_signatures = [
            self.get_command_signature(command) for command in group.commands
        ]
        ...
        if arg_subcmds:
            embed.add_field(
                name=f"Usage", value=f"`_{group.name}` {arg_subcmds}", inline=False
            )
        else:
    ...
    async def send_command_help(self, command):
        ...
        # If user doesn't have permission to use command
            ...
            await self.get_destination().send(
                f'No command called "{command.name}" found.'
            )
